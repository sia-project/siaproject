...
public class ManageAccount extends HttpServlet {
		...
		if(page!=null ||!page.isEmpty()) {
			...
			case "registerAccount" :
				...
				if (civilite.isEmpty()|| nom.isEmpty()|| prenom.isEmpty()||email.isEmpty()||mailExists|| email.contains("yopmail")||!validateMail(email)||mdp.length()<8
					...
					if(mailExists) 
					{
						request.setAttribute("text", request.getAttribute("text")+"<li>Cette adresse mail est deja utilis�e</li>");
						request.setAttribute("email", "");
					}
					if(prenom.isEmpty()) 
					{
						request.setAttribute("text", request.getAttribute("text")+"<li>Veuillez saisir votre pr�nom</li>");
					}
					if(nom.isEmpty()) 
					{
	...
	private static void sendAccountValidationEmail(String email) 
	{
		try {
			...
			message.setSubject("Compte cr�e sur La Vieille Sardine");
	...
	private static String getMessageHTMLUser(String email) {
		...
		sb.append("			<title>Votre compte a bien �t� cr�e</title>");
		...
		sb.append("							<tr>");
		sb.append("								<td>");
		sb.append("									Votre compte � bien �t� cr�e, vous pouvez d�s � present valider votre adresse email afin d'activer votre compte en cliquant sur le le lien suivant" );
